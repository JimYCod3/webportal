...
public class VenuesController {
    ...
    public String update(ModelMap m, @PathVariable("id") Integer id) throws MetrixException {
        ...
        // if venue with id not found, throw exception, include redirect url to
        // /venues/index in error page
        if (!oVenue.isPresent()) {
            throw new MetrixException(-1, String.format("Venue with id {%d} not found!", id),
                    "/" + VIEW_PREFIX + "index");
        }
    ...
    public String update(ModelMap m, @PathVariable("id") Integer id,
            @Valid @ModelAttribute("updVenue") Venues updVenues, BindingResult result) throws MetrixException {
        Optional<Venues> oVenue = repo.findById(id);
        // if venue with id not found, throw exception, include redirect url to
        // /venues/index in error page
        if (!oVenue.isPresent()) {
        ...
        // if there is reduction of number of seat, while this venue linked to any
        // events, throw exception, include redirect url to /venues/update/{id} in error
        // page
    ...
    public String delete(@PathVariable("id") Integer id) throws MetrixException {
        ...
        Venues venues = oVenue.get();
        // if this venue linked to any events, throw exception, include redirect url to
        // /venues/index in error page
        if (!venues.getEvents().isEmpty()) {
            throw new MetrixException(-1,
                    "Venue already linked with event(s), delete is not allowed! Please delete corresponding event(s) first",
                    "/" + VIEW_PREFIX + "index");
        }
        repo.deleteById(id);
        return "redirect:/" + VIEW_PREFIX + "index";
