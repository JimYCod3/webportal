...
public class EventsController {
    ...
    public String create(ModelMap m, @Valid @ModelAttribute("newEvent") Events newEvents, BindingResult result) {/*
        ...
        if (result.hasErrors()) {/*
                                  ...
                                  */
            m.addAttribute("newEvent", newEvents);
    ...
    public String update(ModelMap m, @PathVariable Integer id) throws MetrixException {/*
                                                                                        * 13. Annotation for mapping the
                                                                                        * variable "id" in the path
                                                                                        */
    ...
    public String update(ModelMap m, @PathVariable Integer id, @Valid @ModelAttribute("updEvent") Events updEvent,
            BindingResult result) throws MetrixException {/* 13. Annotation for mapping the variable "id" in the path */
        /* 9. Annotation for hibernate validation against updEvent object */ /*
                                                                              * 10b. Annotation for mapping HTML from
                                                                              * body with name "updEvent"
                                                                              */
        /* 14. Find the Events from DB, if not found, throw the following exception */
        Optional<Events> event = repo.findById("id");
        if (!event.isPresent()) {
         ...
         */
        if (event.get().getVenue().getId() != updEvent.getVenue().getId() && event.get().isStartSell()) {
    ...
    public String delete(@PathVariable Integer id) throws MetrixException {/*
                                                                            * 13. Annotation for mapping the variable
                                                                            * "id" in the path
                                                                            */
        /* 14. Find the Events from DB, if not found, throw the following exception */
        Optional<Events> event = repo.findById("id");
